...
namespace libraryAutomation
{
    public partial class frmEscrowedBook : Form
    {
        ...
        private void cmboxSelection_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                SqlDataAdapter adapter = new SqlDataAdapter("select * from delayed order by name", con);
                adapter.Fill(table);
                dgwBook.DataSource = table;
            }
            else if (cmboxSelection.SelectedIndex == 2)
            {
                SqlDataAdapter adapter = new SqlDataAdapter("select * from undelayed order by name", con);
                adapter.Fill(table);
                dgwBook.DataSource = table;
            }
        }
